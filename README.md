...
| POST | `/v1/legal-matter-advisory-session/initiate` | Initiate |
| GET | `/v1/legal-matter-advisory-session` | Retrieve (list) |
| GET | `/v1/legal-matter-advisory-session/{crId}/retrieve` | Retrieve |
| PUT | `/v1/legal-matter-advisory-session/{crId}/update` | Update |
| PUT | `/v1/legal-matter-advisory-session/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/legal-matter-advisory-session/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-legal-services:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-legal-services ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-legal-services` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
